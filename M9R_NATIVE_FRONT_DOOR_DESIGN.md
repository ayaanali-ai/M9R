...
## 20. N1 scope additions (2026-09-20, owner: "include those in N1")
The onboarding surfaces are part of N1, built from **one step list** (a small module the CLI, docs page, dashboard card and README all read, so they cannot disagree): (a) `m9r init` as the single user-level guided command (`connect` already exists but is missing from `--help`; document it and make `init` the front door); (b) `m9r doctor` as a live checklist with the one-time steps (Codex `/hooks` trust, Claude Desktop restart) and their fixes; (c) `m9r uninstall`; (d) a `/docs/get-started` page; (e) a first-login dashboard onboarding card with live status; (f) README "Connect your agents" updated. Homepage placement and copy are the designer's: see `M9R_GET_STARTED_FOR_ASTRA.md`, which is the hand-off file for now. (d) and (e) can wait until the designer's new site lands; the step list, CLI and README do not.

## 21. Local product shape and launch mode (decided 2026-09-20)
**What the user runs locally:** the M9R Node (a background service), the CLI, and the hooks and MCP tools it installs. **No TUI** (rejected: that is Herdr's territory and it would make users work inside M9R). **Local viewer:** the Node serves a small page on `127.0.0.1` (opened with `m9r open`) that shows the local timeline, endpoints, tasks and locks with no account. This is what makes local-only mode useful and it replaces the overlay for the first release. **Overlay:** yes, but an optional download after N1 to N3, Windows first. **Installer or desktop app (Electron or Tauri):** not for the first release; only when the overlay ships, as one package bundling the Node, overlay and login service for non-technical users; code-signing cost is deferred until then. **First-release audience:** developers using Claude Code, Codex and OpenCode; install is `npm i -g m9r-cli`.
**Two modes:** *Local mode*: no account, no network needed, everything on this machine. *Connected mode*: signed in; adds the hosted dashboard, multi-machine sync, phone approvals and, later, teams and cross-user.
**Launch mode (waitlist):** hybrid. The homepage has a waitlist form (email plus which agents the person uses). Anyone can install the local layer from npm with no account (it is public and works alone). Hosted accounts open in invite waves: wave 1 is 10 to 20 technical users you know, then widen. Lift the gate when Stage 0 reliability and slices N1 to N3 pass and the demo video exists. The waitlist page must show the demo, not just a form.

## 22. Waitlist decisions (2026-09-20)
Owner keeps the waitlist (it buys time for the overlay and a packaged installer; a select few get the local viewer page). Free is shown as "$0.00, access opening soon" with a "Request early access" action, not as usable now, because N1 is not built. Waitlist signups use **double opt-in**: store the address immediately as unconfirmed, send one confirmation email, count and invite only confirmed addresses; record consent time and source; every email carries a working unsubscribe link (Cloudflare Email Service does not process unsubscribes, so the app keeps its own suppression flag). **No email is sent until a domain we control exists.** Facts checked 2026-09-20: `m9r.ai`, `m9r.dev`, `m9r.app` and `m9r.io` return "not found" at their registries (unregistered), `m9r.com` is registered by someone else; `*.workers.dev` cannot be used for email because we do not control its DNS; Cloudflare Email Sending is public beta, needs the sending domain on Cloudflare DNS and the Workers Paid plan, includes 3,000 emails per month then $0.35 per 1,000, and inbound Email Routing is free and unlimited; Cloudflare Registrar sells `.ai` at cost. Sender plan: from `access@<domain>` (display name "M9R"), reply-to `hello@<domain>`, inbound routed to the owner's inbox. The app already has a Resend integration in `src/lib/email.ts` as a fallback path.

## 23. N1 status (2026-09-20): Claude side built and passing
Shipped in N1: the tiny hook program (`cli/dist/m9r-hook.js`), the local store, the hook handler, and the commands `m9r setup` (with `--dry-run`, `--yes`, `--status`), `m9r uninstall` (`--purge`), and `m9r send @<agent> "<message>"`, plus a native section in `m9r doctor` and `connect`/`setup`/`uninstall`/`send` in `--help`. **Naming decision:** the user-level command is `m9r setup` for now, not `m9r init`, because `init` is the existing per-repo, human-approved claim flow and 141 tests depend on it. The intended end state (one `init` that signs in, approves the device and then runs the local setup) is a later slice; until then the docs and the designer file say `m9r setup`. Two real-Claude findings changed the design: (1) delivery is **once per agent, not once per window** (a per-window cursor made every new Claude window repeat an already-delivered task); (2) the inbox line adds the `get_task` pointer only when the goal was truncated (there is no `get_task` tool yet, and Claude reported "task not found" when pointed at one), and the acknowledgement now says to tell the user the task was handed to the target. Acceptance (`npm run n1:acceptance`, real `claude -p`, scratch project, no account): idle prompt injects nothing; a task from `m9r send` is handled at the next prompt and not repeated; a task awaiting approval is not acted on; a typed `@codex` creates a task and the sender says it was handed over instead of doing it. Not covered: Claude Desktop (needs the app), and delivery to Codex (N2). Tests: 58 native unit tests, all in the main `test` script.

## 24. Local viewer removed (2026-09-20)
The localhost viewer page (`m9r open`, viewer-core, viewer-server and its tests) was built, checked in a browser, and then **removed at the owner's direction**: it is not something to hand to users. Section 21's "local viewer" line no longer applies. The hosted web app is the viewer until the overlay ships; the overlay comes before the Tauri installer. The store's event log and delivery timestamps were kept because the web sync (`M9R_NEXT_BUILD_PLAN.md`, W1c) needs them. See that file for the web wiring and the build order.
