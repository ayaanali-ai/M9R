...
test("mergeClaudeCodeLocalSettings preserves the user's existing hooks and settings untouched", () => {
  ...
  const sessionEndGroups = parsed.hooks.SessionEnd as Array<{ hooks?: Array<{ command?: string }> }>;
  assert.ok(sessionEndGroups.some((group) => group.hooks?.some((hook) => hook.command === "my-own-script.sh")));
});
