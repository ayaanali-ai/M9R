...
function runtimeSandbox() {
  ...
  for (const f of ["m9r-hook.js", "local-store.js", "hook-handler.js", "inbox-core.js", "mention-core.js"]) writeFileSync(join(source, f), `// ${f} v1\n`, "utf8");
  const out: string[] = [];
  const err: string[] = [];
...
test("setup copies the hook program into ~/.m9r/bin and points the hooks there, not at where the CLI happens to live", async () => {
  ...
  assert.equal(await s.run("setup", ["--yes"]), 0);
  for (const f of ["m9r-hook.js", "local-store.js", "hook-handler.js", "inbox-core.js", "mention-core.js", "package.json"]) assert.equal(existsSync(join(s.bin, f)), true, f);
  ...
  assert.equal(JSON.parse(readFileSync(s.p.manifest, "utf8")).runtimeFiles.length, 6);
  s.done();
});
