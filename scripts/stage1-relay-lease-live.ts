...
type LiveFrame = { type?: string; [key: string]: unknown };
const parseFrame = (value: unknown): LiveFrame => {
  const parsed: unknown = JSON.parse(String(value));
  return parsed && typeof parsed === "object" ? parsed as LiveFrame : {};
};

function open(label: string) {
  const ws = new WebSocket(`wss://${RELAY}/`);
  const state = { label, closed: null as null | { code: number; reason: string; atMs: number }, frames: [] as LiveFrame[], openedAt: Date.now() };
  ws.on("message", (d) => state.frames.push(parseFrame(d)));
  ws.on("close", (code, reason) => { state.closed = { code, reason: String(reason), atMs: Date.now() - state.openedAt }; });
...
const beat = (c: { ws: WebSocket }, id: string) => c.ws.send(frame("bridge.heartbeat", { protocolVersion: "oathlock.bridge.v1", bridgeInstanceId: id, activeSessionIds: [] }));
...
check("the server told it why before closing (presence_lease_expired error frame)", silent.state.frames.some((f) => {
  const payload = f.payload;
  return payload && typeof payload === "object" && (payload as { code?: unknown }).code === "presence_lease_expired";
}));
