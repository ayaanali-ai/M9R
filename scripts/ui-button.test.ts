...
test("Button renders the shared .ol-btn classes and defaults to type=button", () => {
  const html = renderToStaticMarkup(createElement(Button, { variant: "primary", size: "sm", children: "Save" }));
...
test("loading disables activation, announces busy and shows the spinner", () => {
  const html = renderToStaticMarkup(createElement(Button, { loading: true, children: "Send" }));
...
test("IconButton always carries an accessible name and a tooltip", () => {
  const html = renderToStaticMarkup(createElement(IconButton, { label: "Close panel", children: "x" }));
