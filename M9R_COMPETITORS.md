...
### Mosaic (mosaic.inc): correction and verified facts (2026-09-20)
Earlier text here said Mosaic had "no evidence of live messaging". **That was incomplete.** Verified 2026-09-20 from their own pages: (1) the GitHub repo `emergent-inc/mosaic` is "a native macOS terminal workspace built for collaborating with people and agents at the same time": real-time sharing of terminals and Claude Code sessions, teammates watch and control shared panes with live cursors, over a Cloudflare-based relay, with native hooks for agents like Claude Code; Swift/AppKit on libghostty; **GPL-3.0-or-later** (a genuine open-source licence), derived from cmux. (2) The company (YC Summer 2026, two founders) sells shared memory across sessions: "what was tried, what changed, and why", synced from Claude Code, Codex and Cursor; about 2,000 installs and 37 organisations syncing, self-reported. (3) Install: `brew install emergent-inc/tap/mosaic && mosaic install`, so macOS-first. (4) YC's page mentions two demo videos (a multiplayer IDE and agent session sync). **Not verified:** I cannot watch the videos, so the claim that the demo shows a visible per-person, per-project transcripts folder (`.mosaic/People/<name>/<project>/agent-transcripts/`) rests on our own earlier note in `memory-export-core.ts` and the owner's recollection, not on a source I could open today. Implication: they overlap us on live shared terminals and on shared memory, and they are open source under GPL; do not claim we have live collaboration they lack. Threat level raised from Medium to Medium-High.
...
### Mosaic demos, watched in the browser pane (2026-09-20)
Both demos are on YC's launch page for Mosaic (YouTube: `SQywbNRe2xk`, 49 s; `QVzn-1vzpsY`, "Mosaic Demo 2", 27 s). What they show, frame by frame:
- **Demo 1 (terminal app):** two teammates' Claude Code sessions side by side in the Mosaic macOS terminal, with named live cursors (Shubham Patil, Nick Williams) like a shared document. One person's message ("the british are coming") is relayed to the other person's session: the second person asks "what is the message?" and their Claude answers it. Claude's own reply mentions sending it "to that other terminal pane in the room ... via the mosaic handoff command". Then a **web sessions list** (owner, time, prompt; entries such as "what was dorsa working on for regarding model training"), then an agent that says it loaded a skill, "looks up the sessions reference and searches the catalog", and extracts a teammate's Codex transcript. The local path in that frame is too small to read, so the claim that the demo shows a `.mosaic/People/<name>/<project>/agent-transcripts/` folder remains **unverified from the video**.
- **Demo 2 (web app):** workspace "Ocean Test Team" with left navigation **Sessions, People, Integrations**, members with roles, a table of every teammate's sessions across tools (Claude and Cursor icons) with prompts and owners, and a session detail page with the full transcript including tool calls (file reads) and a metadata sidebar (agent, model, project, owner, visibility, started).
- **Their manifesto** ("The Coordination Problem", mosaic.inc/blog/memo, June 2026) argues against agents messaging each other like an org chart and for one shared, continuously updated context store that ephemeral agents read and write ("the session is not the unit; the store is").
Implications for us: the visible "place where memory lives" in their demo is the web Sessions and People catalog with full transcripts, plus a skill that lets an agent search it. Our dashboard Memory page shows learned rules and findings, not a cross-person, cross-tool session catalog. Their live message handoff exists but is inside their own terminal app; ours (typed `@mention` into any plain Claude or Codex session) does not require their terminal, but is not yet built for Codex or shown in the web.
