...
### 2a. Liveness of Codex sessions (built 2026-09-20)
Codex has no session-end hook and its hook payload has no process id, but a live session holds its rollout file open. Windows Restart Manager (`src/lib/native/codex-liveness.ts`) reports which processes hold a file without opening it exclusively, so it cannot disturb Codex. Verified on a real interactive session (held while live, free after exit or kill) and on the Codex Desktop app (the active Desktop thread's file is held by the app). When a push has to choose between sessions it asks once (about 1.2 s): open sessions are the candidates; one open session is used; two open ones are narrowed by the sender's folder and otherwise never guessed (the task falls back to the inbox; `m9r-cli sessions` and `send --session` aim it). **Windows only:** other platforms report "unknown" (treated as no information, never as closed); a Mac/Linux equivalent (probably `lsof`) is untested. `npm run n2:live -- --two --both-live` proves it against real interactive sessions. Also fixed here: a prompt M9R itself pushed is handled alone, so an older inbox item cannot be mixed into it.
