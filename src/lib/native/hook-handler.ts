...
export function handleHookEvent(input: HookInput, ctx: HookContext): AdditionalContext | null {
  try {
    ...
    if (event === "SessionStart") {
      ...
      // Only mention memory if the folder really exists here (the earlier card pointed at a file that did not).
      const memoryDir = ctx.memoryDir ?? (input.cwd && (ctx.pathExists ?? existsSync)(join(input.cwd, ".oathlock", "memory")) ? ".oathlock/memory" : undefined);
